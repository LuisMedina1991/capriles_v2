...
class OtherProviders extends Component
{
    ...
    public function mount(){
        ...
        $this->componentName = 'otros proveedores';
        //$this->my_total = 0;
        $this->details = [];
    ...
    {   
        ...
        if(strlen($this->search) > 0){
            ...
            $data = OtherProvider::where('description', 'like', '%' . $this->search . '%')
            ->orWhere('reference', 'like', '%' . $this->search . '%')
            ->orderBy('reference', 'asc')
            ->paginate($this->pagination);

            $this->my_total = OtherProvider::where('reference', 'like', '%' . $this->search . '%')
            ->sum('amount');

        }else{

            $data = OtherProvider::orderBy('reference', 'asc')->paginate($this->pagination);

            $vars = OtherProvider::all();
        ...
        return view('livewire.other_provider.other-providers', [
    ...
    public function Store(){
        ...
        if($this->cov_det != null){
            ...
            $rules = [
                ...
                'reference' => 'required|min:5|max:70',
            ...
            $messages = [
                ...
                'reference.max' => 'La referencia debe contener 70 caracteres como maximo',
    ...
    public function Edit(OtherProvider $other){
    ...
    public function Update(){
        ...
        if($this->cov_det != null){
            ...
            $other = OtherProvider::find($this->selected_id);
    ...
    public function Destroy(OtherProvider $other){
    ...
    public function Details(OtherProvider $other){
    ...
    public function Cancel(Detail $det){
        ...
        if($this->cov_det != null){
            ...
            $other = OtherProvider::firstWhere('id',$det->detailable_id);
