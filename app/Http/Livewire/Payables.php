...
class Payables extends Component
{
    ...
    public function mount(){
        ...
        $this->componentName = 'otros por pagar';
        //$this->my_total = 0;
        $this->details = [];
    ...
    {   
        ...
        if(strlen($this->search) > 0){
            ...
            $data = Payable::where('description', 'like', '%' . $this->search . '%')
            ->orWhere('reference', 'like', '%' . $this->search . '%')
            ->orderBy('id', 'asc')
            ->paginate($this->pagination);

            $this->my_total = Payable::where('reference', 'like', '%' . $this->search . '%')
            ->sum('amount');

        }else{

            $data = Payable::orderBy('reference', 'asc')->paginate($this->pagination);
        ...
        return view('livewire.payable.payables', [
    ...
    public function Update(){
        ...
        if($this->cov_det != null){
            ...
            $payable = Payable::find($this->selected_id);
            ...
            $messages = [
                ...
                'description_2.required' => 'El detalle es requerido',
    ...
    public function Details(Payable $payable){
    ...
    public function Cancel(Detail $det){
        ...
        if($this->cov_det != null){
            ...
            $payable = Payable::firstWhere('id',$det->detailable_id);
